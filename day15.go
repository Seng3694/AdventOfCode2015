...
func day15_parse_line(str string) (ingredient CookieIngredient) {
...
func min_clamp(value, min int) int {
	if value < min {
		return min
	} else {
		return value
...
func calculate_cookie_score(amount []IngredientAmount, calorieConstraint int) (score int) {
	...
	result.capacity = min_clamp(result.capacity, 0)
	result.durability = min_clamp(result.durability, 0)
	result.flavor = min_clamp(result.flavor, 0)
	result.texture = min_clamp(result.texture, 0)

	return result.capacity * result.durability * result.flavor * result.texture
}

func day15_solution_recursive(calorieConstraint, index, remaining int, amounts *[]IngredientAmount, maxScore *int) {
	...
	for i := 0; i <= remaining; i++ {
		...
		day15_solution_recursive(calorieConstraint, index+1, r, amounts, maxScore)
...
func day15_solution(calorieConstraint int) int {
	...
	score := 0
	day15_solution_recursive(calorieConstraint, 0, 100, &amounts, &score)
	return score
}
...
func day15() (string, string) {
	...
	for scanner.Scan() {
		line := scanner.Text()
		ingredients = append(ingredients, day15_parse_line(line))
	}

	return fmt.Sprint(day15_solution(0)), fmt.Sprint(day15_solution(500))
}
