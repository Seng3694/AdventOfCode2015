...
func day8() (string, string) {
	...
	var (
		visualTotal, memoryTotal, encodedTotal int
	)

	for scanner.Scan() {
		line := scanner.Text()
		visual, memory, encoded := day8_count_string_memory(line)
		fmt.Printf("%v: %v %v %v \n", line, visual, memory, encoded)
		visualTotal += visual
		memoryTotal += memory
		encodedTotal += encoded
	}

	return fmt.Sprint(visualTotal - memoryTotal), fmt.Sprint(encodedTotal - visualTotal)
}
